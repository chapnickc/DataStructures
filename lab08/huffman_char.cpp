...
double calculate_entropy(HuffNode<char>* root, double H, int total){
  if (root->isLeaf()){ 
  ...
  return H + 
    calculate_entropy(((IntlNode<char>*)root)->right(), H, total) + 
    calculate_entropy(((IntlNode<char>*)root)->left(), H, total);
}
